...
// bloqueia novo pedido a partir de agora (createOrder exige status OPEN).
// requestClosingAction não segue o padrão FormState (redireciona no
// servidor ou lança o erro de domínio pro error boundary — mesma escolha
...
export function RequestClosingButton({
  ...
  const action = requestClosingAction.bind(null, redirectPath, sessionId);
    ...
    <>
      ...
      <form action={action}>
        <ConfirmDialog
          ...
          description={`${itemLabel} deixa de aceitar novos pedidos e entra na etapa de fechamento (taxa, desconto e pagamento).`}
          cancelLabel="Voltar"
